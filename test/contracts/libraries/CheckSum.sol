...
library CheckSum {
    ...
    function toChecksumString(
        address addr
    ) internal pure returns (string memory addrStr) {
        addrStr = string(abi.encodePacked("0x", getChecksum(addr)));
    }

    /// @notice Obtains the checksum of an address
    /// @param account The address to be processed
    ...
    /// @notice Obtains the checksum capitalized characters of an address
    /// @param account The address to be processed
    ...
    /// @notice Checks if a checksum is valid
    /// @param accountChecksum The checsum to ve validated
    /// @return ok True if the checksum is valid, false otherwise
    ...
    function _isChecksumValid(
        ...
        return (keccak256(abi.encode(actual)) ==
            keccak256(abi.encode(provided)));
    }
    ...
    function _toAddress(
        ...
        if (accountBytes.length == 40) {
            ...
            bytes memory packed = abi.encode(accountAddressBytes);
            assembly {
